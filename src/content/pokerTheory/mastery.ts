...
export function isPokerTheoryLessonUnlocked(
  lessonId: string,
  progress: CourseProgress,
): boolean {
  const idx = getPokerTheoryLessonIndex(lessonId);
  if (idx <= 0) return true;
  const prev = POKER_THEORY_LESSONS[idx - 1];
  return Boolean(progress.lessonMastery[prev.lessonId]?.passed);
}

export function roundForPokerTheoryLesson(
  lessonId: string,
  progress: CourseProgress,
): number {
...
export function buildPokerTheoryAttemptQuestions(
  lesson: Lesson,
  round: number,
): RenderableQuestion[] {
  if (round <= 0) {
